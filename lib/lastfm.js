...
function LastFM(options){
  ...
  var apiKey    = options.apiKey    || '645f34cc469fc92baa8d2ab23214a36b';
  var apiSecret = options.apiSecret || '9c6c9875d868c79365dbe9d365ba6f8d';
  ...
  var internalCall = function(params, callbacks, requestMethod){
    ...
    var purl = apiUrl+ '?' + paramsArray.join('&').replace(/%20/g, '+');
    var r = Request({url:  purl, content: {format: "json"}, overrideMimeType: "application/json",
              onComplete: function(response){
                if(response.statusText == "OK")
                {
                   console.log("complete in request: ", typeof response.json);
                   callbacks.success(response.status, response.json);
                } else {
                  console.log("error in request: ", response.text);
                  callbacks.error(response.status, purl, response.text);
                }
              }
    });
