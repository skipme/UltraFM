(function(){
      ...
      if(tokentab.url.contains("lastfm.ru/home?UFM=UltraFMFF"))
      {
        ...
        lastfm.auth.getSession({token: token}, {
          success: function (responseCode, responseJson) {
            ...
            {
              console.warn("can't create session for lastfm");
              return;
            }
  ...
  function preparePanel() {
      ...
      widgetPanel.port.on("loadXSPF", function(){
            var url = "http://94.25.53.131:80/ultra-128.mp3.xspf";
      ...
      widgetPanel.port.on("lastfm_scrobble", function(obj){
        lastfm.track.scrobble({track: obj.track, artist: obj.artist}, {key: lastfm_state.key},
          {
          success: function (responseCode, responseJson) {
