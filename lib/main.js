(function(){
  ...
  { 
    ...
    m3u: "http://nodes.apphb.com/home/rr?u=4fcc",
    mp3: null,
    xspf: null
  };
  ...
  function preparePanel() {
      ...
      var r = Request({url: radioUrls.m3u, content: {}, overrideMimeType: "application/json",
        onComplete: function(response) {
          if(response.statusText === "OK")
          {
            if(response.text.length > 48) { console.error("unexpected length of server respose on tags request"); return;}
            console.log(response.text)
           widgetPanel.port.emit("here_tags_darling", {code: 200, data: {tags: response.text}});
          }else{
            widgetPanel.port.emit("here_tags_darling", {code: -1, data: {tags: ''}});
          }
        }
      });
      r.get();
